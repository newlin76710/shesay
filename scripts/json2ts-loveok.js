...
const data = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'lib', 'loveok-articles.json'), 'utf-8'));

const lines = [];
lines.push(`export type LoveokBlock =`);
lines.push(`  | { type: 'text'; value: string }`);
lines.push(`  | { type: 'image'; value: string }`);
lines.push(`  | { type: 'heading'; value: string };`);
lines.push(``);
lines.push(`export type LoveokArticle = {`);
lines.push(`  title: string;`);
lines.push(`  heroImage: string;`);
lines.push(`  content: LoveokBlock[];`);
lines.push(`};`);
lines.push(``);
lines.push(`export const loveokArticles: Record<string, LoveokArticle> = {`);
...
const outPath = path.join(__dirname, '..', 'lib', 'loveok-articles.ts');
